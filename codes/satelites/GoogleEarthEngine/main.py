...
Sentinel2 = ee.ImageCollection('COPERNICUS/S2').filterBounds(region).filterDate(parse('2020-11-01'),parse('2020-12-27')).map(cloudMasking).select(['B2','B3','B4','B8','B12'])
imageList = Sentinel2.toList(300) 
for i in range(imageList.size().getInfo()):
    image = ee.Image(imageList.get(i))
    ImageExport(image.reproject(crs='EPSG:4326',scale=10),image.get('system:index').getInfo(),'Vancouver',region['coordinates'][0],10)